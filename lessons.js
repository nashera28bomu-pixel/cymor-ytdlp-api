// =============================================
// CYMOR CODE LEARNER - CORE ENGINE v2.5
// BUG FIX: Modal next-lesson navigation fixed
// =============================================
...
function renderChallengeStep() {
  ...
  if (hintBtn) {
    hintBtn.onclick = () => {
      ...
      if (hintBox) {
        hintBox.textContent = sandbox.mini_challenge.hint || `Try using the <${keyword}> tag.`;
...
function updateLivePreview() {
  ...
  if (preview)
    preview.srcdoc = `<html><style>body{color:black; font-family:sans-serif; padding:15px;}</style><body>${code}</body></html>`;
}

// =============================================
// FIX: finishLesson — modal button no longer
// has hardcoded onclick="location.reload()"
// in the HTML, so this JS handler works correctly.
// =============================================
  ...
  try {
    ...
    if (modal) {
      ...
      if (modalBtn) {
        ...
        // KEY FIX: Completely replace the button to remove any
        // inline onclick handlers, then attach a clean listener.
